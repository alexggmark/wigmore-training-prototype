...
export default function Header() {
  ...
  const trainingRef = useRef<HTMLLIElement | null>(null)

  const [pastBar, setPastBar] = useState(false)
  const [open, setOpen] = useState(false)
  const [hoveringNav, setHoveringNav] = useState(false)

  // Flip header skin after passing the announcement bar
  useEffect(() => {
  ...
  // Hover intent timer to avoid flicker between trigger and dropdown
  ...
  useEffect(() => {
    function onDocDown(e: MouseEvent) {
      ...
      const inTrigger = trainingRef.current?.contains(trg)
      const inDrop = dropdownRef.current?.contains(trg)
      if (!inTrigger && !inDrop) {
        setOpen(false)
  ...
  // Skin logic: white/dark if pastBar, not transparent, or hovering anywhere on header/dropdown
  const skinClass =
    pastBar || !isTransparent || hoveringNav
  ...
  const CaretSvg = () => (
    <svg width="12" height="8" viewBox="0 0 14 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 1L7 7L1 1" stroke="currentColor"/></svg>
  )
  const AccountSvg = () => (
  ...
  return (
    <div className="relative z-40">
      ...
      <header
        ref={headerRef}
        className={`${baseHeader} ${positionClass} ${skinClass}`}
        onMouseEnter={() => setHoveringNav(true)}
        onMouseLeave={() => {
          // Leaving the whole header: close immediately and revert skin (unless pastBar keeps it white)
          clearCloseTimer()
          setOpen(false)
          setHoveringNav(false)
        }}
      >
        <div className={`container mx-auto flex items-center justify-between transition-[padding] duration-200 ${paddingClass}`}>
          <div className="flex items-center gap-12">
            <div className={`${logoWidth} transition-all duration-100`}>
              <LogoSvg />
            </div>

            {/* Primary nav */}
            <nav aria-label="Primary" className="hidden md:block">
              <ul className={`flex items-center gap-8 ${pastBar ? 'text-[13px]' : 'text-[14px]'}`}>
                <li className="link-underline-trigger link-underline">Home</li>
                <li className="link-underline-trigger link-underline">Products</li>

                {/* TRAINING (trigger) */}
                <li
                  ref={trainingRef}
                  className="flex items-center gap-2 cursor-pointer"
                  onMouseEnter={() => { clearCloseTimer(); setOpen(true); setHoveringNav(true) }}
                  onMouseLeave={() => { scheduleClose() }}
                  onFocus={() => { setOpen(true); setHoveringNav(true) }}
                ...
                <li className="link-underline-trigger link-underline">Exclusives</li>
                <li className="link-underline-trigger link-underline">Offers</li>
                <li className="link-underline-trigger link-underline">News</li>
              </ul>
            </nav>
          </div>
          ...
          <div
            className={`flex w-full max-w-52 items-center gap-2 rounded-full border bg-brand-dark/5 px-3 py-1.5
              ${isTransparent && !pastBar && !hoveringNav ? 'border-brand-light' : 'border-brand-dark'}`}
          >
            <SearchSvg />
            <span>Search</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <a className="link-underline-trigger flex flex-row items-center gap-2" href="/login">
              <AccountSvg />
              <span className="text-sm link-underline">Login</span>
            </a>
            <a
              href="/signup"
              className="flex items-center gap-2 rounded-full border border-transparent bg-brand-secondary hover:bg-[#368281] px-4 py-1.5 text-brand-light"
            >
              <span>Sign up</span>
        ...
        {/* Dropdown panel (expanding animation) */}
        <div
          ref={dropdownRef}
          className={`absolute top-[calc(100%+1px)] w-full border-t border-brand-border bg-brand-light text-brand-dark shadow-lg
            overflow-hidden origin-top transform-gpu will-change-[transform,opacity]
            transition-[opacity,transform] duration-200 ease-out
            ${open ? 'opacity-100 scale-y-100 visible pointer-events-auto' : 'opacity-0 scale-y-60 invisible pointer-events-none'}`}
          onMouseEnter={() => { clearCloseTimer(); setOpen(true); setHoveringNav(true) }}
          onMouseLeave={() => { scheduleClose(); setHoveringNav(false) }}
        >
          <div className="w-full container mx-auto py-12">
            <div className="grid grid-cols-3 gap-12">
              <ul className="flex flex-col gap-6">
                {/*  className="link-underline-trigger link-underline" */}
                <li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li>
              </ul>
              <ul className="flex flex-col gap-6">
                <li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li><li className="link-underline-trigger"><span className="link-underline">Item</span></li>
              </ul>
